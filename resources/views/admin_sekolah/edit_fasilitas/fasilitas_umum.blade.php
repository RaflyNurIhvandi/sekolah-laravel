...
@section('konten')
    ...
    @foreach ($umum as $i)
        <form action="/update/fasilitas/umum/{{ $i->no_fasilitas }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3" style="width: 50%">
                <label for="deskripsi" class="form-label">keteranngan</label>
                <textarea name="keterangan" id="keterangan" class="form-control" cols="30" rows="2">{{ $i->keterangan }}</textarea>
            </div><br>
            <div>
