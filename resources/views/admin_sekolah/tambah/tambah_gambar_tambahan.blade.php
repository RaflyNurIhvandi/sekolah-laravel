...
@section('konten')
    ...
    @foreach ($editgmt as $i)
        <div class="tambah-team" id="tambah-team-{{ $i->no_gambar }}">
            <div class="form-tambahteam">
                ...
                <form action="/update/tambahan/{{ $i->no_gambar }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="text-center">
                        <div class="profil"><br class="br1" id="br1"><br class="br2" id="br2_{{ $i->no_gambar }}">
                            <center>
                                <span class="war-foto" id="war_foto_{{ $i->no_gambar }}">Masukan Kembali Gambar Untuk Mengganti Foto!</span>
                            </center>
                            <div class="text-center">
                                <center>
                                    <img src="/gambar/fasilitas/tambahan/{{ $i->gambar }}" class="gambar" id="preview_{{ $i->no_gambar }}" data-id-inp="gambar_{{ $i->no_gambar }}" onclick="previewImg(this)" width="260" height="260">
                                    <input type="file" name="gambar" id="gambar_{{ $i->no_gambar }}" onchange="gambarInput(this)" data-br="br2_{{ $i->no_gambar }}" data-war="war_foto_{{ $i->no_gambar }}" data-id-prev="preview_{{ $i->no_gambar }}" data-id-gbr="gambar_{{ $i->no_gambar }}" hidden>
                                </center>
                            </div>
                        </div><br>
                    </div>
                    <center>
                        <button type="button" class="btn btn-primary btn-sm" onclick="sa(this)" data-id-sv="ve_{{ $i->no_gambar }}">Simpan</button>
                        <button type="submit" id="ve_{{ $i->no_gambar }}" hidden></button>
                        <button type="reset" class="btn btn-secondary btn-sm" onclick="resetNoHid(this)" data-war="war_foto_{{ $i->no_gambar }}" data-br="br2_{{ $i->no_gambar }}" data-prev="preview_{{ $i->no_gambar }}">Reset</button>
                        <button type="button" class="btn btn-danger btn-sm" onclick="del_gam_fas(this)" data-del="delete_{{ $i->gambar }}">Delete</button>
                        <a href="/delete/gambar/fasilitas/tambahan/{{ $i->no_gambar }}" id="delete_{{ $i->gambar }}" hidden></a>
                    </center>
                </form>
            </div>
        </div>
            ...
            @foreach ($editgmt as $i)
                <div class="col">
                    <div class="profil"><br class="br1" id="br1"><br class="br2" id="br2">
                        <img src="/gambar/fasilitas/tambahan/{{ $i->gambar }}" class="gambar" id="preview" width="260" height="260" onclick="tambahTeam(this)" data-id-update="tambah-team-{{ $i->no_gambar }}">
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="tambah-umum" id="tambah-umum">
        <div class="form-tambahumum">
            ...
            <form action="/save/fasilitas/tambahan" method="POST" enctype="multipart/form-data">
                @csrf
                @foreach($addgmt as $a)
...
<script>
    ...
    function previewImg(obj) {
        document.getElementById(obj.getAttribute('data-id-inp')).click()
    }

    function gambarInput(obj) {
        document.getElementById(obj.getAttribute('data-war')).style.display = 'none'
        document.getElementById(obj.getAttribute('data-br')).style.display = 'block'
    ...
    function resetNoHid(obj) {
        document.getElementById(obj.getAttribute('data-war')).style.display = 'block'
        document.getElementById(obj.getAttribute('data-br')).style.display = 'none'
        document.getElementById(obj.getAttribute('data-prev')).src = '/gambar/preview.jpg'
    }
