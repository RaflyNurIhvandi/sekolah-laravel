...
@section('konten')
        ...
        <div class="kotak-profil mt-3">
            @foreach ($profile as $i)
                <div class="text-center">
                    ...
                    <button type="button" class="btn btn-outline-primary btn-sm" style="width: 100px; color:black;" onclick="resign_guru()">Resign</button>
                </div>
            @endforeach
        </div>
        @foreach ($profile as $i)
    ...
    <div hidden>
        @foreach($rsg as $i)
            <form action="/alasan/resign/{{ $i->no_status }}" method="POST">
                @csrf()
                <input type="text" name="alasan" id="alasan">
                <button type="submit" id="kirim-alasan-resign"></button>
            </form>
        @endforeach
    </div>
@endsection
